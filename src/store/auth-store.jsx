...
import { create } from 'zustand';
import { persist } from 'zustand/middleware';
import { actionLogin } from '../api/auth'; // ตรวจสอบ path ให้ถูกต้อง
      ...
      (set, get) => ({
         ...
         fetchCustomerId: async () => {
            try {
               ...
               const res = await axios.get(`http://localhost:8008/api/user/users/${id}`, {
                  headers: { Authorization: `Bearer ${token}` },
               });
         ...
         actionLogout: () => {
            set({ user: null, token: '', customerId: null, firstname: '', lastname: '', address: '' });
         },
      }),
      {
         name: 'auth-store',

...
const PlaceOrdercopy2 = () => {
   ...
   useEffect(() => {
      // Get token directly inside useEffect
      const { token } = useAuthStore.getState();
      
      if (!token) {
         // Redirect to login immediately and don't set authenticated state
         navigate("/login");
      } else {
         // Only set authenticated to true if we have a token
   ...
   const handlePlaceOrder = () => {
      // Check token again before proceeding with order
      const { token } = useAuthStore.getState();
      if (!token) {
         navigate("/login");
         return;
      ...
      // Continue with order placement if we have a token
      navigate("/orders");
   };
...
export default PlaceOrdercopy2